...
- Gardens without a biome backdrop (1–10 and the Crown) draw the Sanctuary backdrop: five 320×180 parallax layers in `assets/backdrop-v1/` (sky, band, mountains, ruins, forest; built by `scripts/build-backdrop.py` from `docs/asset-review/sanctuary-backdrop-v1/`), tiled with mirrored alternate tiles at the rates in `SANCTUARY_PAR`. Frost and ember gardens keep their biome backdrops. Until the layers load the old sky and hills stand in (`tests/backdrop.test.cjs`).
