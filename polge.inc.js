...
function polgeBurst(q){
  ...
  function burst(){
    ...
    if(q.raincoat)gardenPlots.forEach(function(p){if(!p.dead&&Math.hypot(p.x-q.x,surfaceY(p.x)-q.y)<=r){p.moisture=clamp01(p.moisture+.18*q.raincoat);p.pulse=Math.max(p.pulse,.8);}});
