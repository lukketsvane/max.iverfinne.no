...
test('the garden collection lists all twenty plant types and marks the ones grown to full size', () => {
  const g = loadGame().game; g.resetRogueRun('test', { classId: 'mech', skinId: 'original' });
  assert.deepEqual([...g.plantCollection().map(k => k.found)], Array(20).fill(false));
