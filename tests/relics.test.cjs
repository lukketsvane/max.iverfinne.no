...
test('only the signed-in owner receives all relics; future grants are per account and cannot come from a display name',async()=>{
  ...
  assert.deepEqual(relicCollection({id:'owner',email:'lukketsvane@players.max.invalid'}).map(r=>r.id),['last-seed']);
  ...
  assert.deepEqual(relicCollection({id:'other',email:'someone@players.max.invalid'},['relic-last-seed']).map(r=>r.id),['last-seed']);
});
