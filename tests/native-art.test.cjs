...
test('native assets load once, independent failures keep the other skins and enemies usable', async () => {
  ...
  assert.deepEqual([...status.failed], ['moss']); assert.equal(status.loaded.length, 24);
  ...
  assert.equal((await art.load()).loaded.length, 24); assert.equal(requests.length, 25);
...
test('Sligo\'s pack loads on demand, never with the others, and until it loads the original Max stands in', async () => {
  ...
  assert.equal(status.loaded.length, 25); assert.equal(requests.some(url => url.includes('/sligo/')), false);
