...
test('fresh Sligo plants draw red clots instead of ordinary dark seeds, with a red loading fallback', () => {
  ...
  g.plantAtlasReady=true;
  g.ctx={drawImage(...args){images.push(args);},fillRect(){inks.push(this.fillStyle);}};
  fx.complete=false;
  for(const kind of [25,26])g.drawGrowingFigmaPlant(plot({kind,growth:0,seed:3}),20,40,0);
  assert.equal(images.length,0);assert.ok(inks.includes('#8f263a'));assert.ok(inks.includes('#dc7470'));
  fx.complete=true;fx.naturalWidth=1600;fx.naturalHeight=40;
  for(const kind of [25,26])g.drawGrowingFigmaPlant(plot({kind,growth:.05,seed:3}),20,40,0);
  assert.equal(images.length,2);
  for(const call of images){assert.equal(call[0],fx);assert.equal(call[1],29*40);}
  g.drawGrowingFigmaPlant(plot({kind:0,growth:0,seed:3}),20,40,0);
  assert.notEqual(images.at(-1)[0],fx,'ordinary seeds keep their artwork');
});
