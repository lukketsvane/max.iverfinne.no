...
test('every one of the twenty-seven plant kinds (Sligo\'s two included) has a feature and a rarity tier', () => {
  const g = game();
  assert.equal(g.PLANT_FEATURES.length, 27); assert.equal(g.PLANT_TIER.length, 27);
