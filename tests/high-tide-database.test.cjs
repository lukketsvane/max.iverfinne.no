...
for(const mode of ['high-tide','night-relay'])test(mode+' admits new guests without unlocks and preserves mode, character and invite isolation',async()=>{
 ...
 try{
  ...
  const join=async(c,m=mode)=>(await db.query('select public.max_coop_global($1,$2,$3) r',[c,'easy',m])).rows[0].r;
  await as(null);assert.equal((await db.query(`select public.max_coop_status('${mode}') r`)).rows[0].r.active,false);await assert.rejects(join('mech'),{code:'42501'});
  await as(0);const tide=await join('polge');assert.equal(tide.mode,mode);assert.equal(tide.difficulty,'easy');await assert.rejects(join('polge','last-seed'),{code:'42501'});
  ...
  const joined=(await db.query('select public.max_coop_global($1,$2,$3,$4) r',['runner','hard',mode,tide.id])).rows[0].r;assert.equal(joined.id,tide.id);assert.equal(joined.difficulty,'easy');assert.equal(joined.members.length,2);
  const status=(await db.query(`select public.max_coop_status('${mode}') r`)).rows[0].r;assert.equal(status.id,tide.id);assert.equal(status.players,2);
  ...
  await as(1);const poll=(await db.query("select public.max_coop('get',jsonb_build_object('room',$1::text)) r",[tide.id])).rows[0].r;assert.equal(poll.mode,mode);
