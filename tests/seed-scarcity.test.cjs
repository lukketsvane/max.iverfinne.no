...
test('seeds are scarce, and the two gardening classes find almost twice as many', () => {
  assert.deepEqual(classes.all.map(c => [c.id, c.seeds]), [['mech', .4], ['runner', .7], ['bulwark', .4], ['herbalist', .7]]);
  const found = {};
  ...
  assert.deepEqual(found, { mech: 8, runner: 14, bulwark: 8, herbalist: 14 }, 'the remainder carries over, so nothing is lost to rounding');
});
