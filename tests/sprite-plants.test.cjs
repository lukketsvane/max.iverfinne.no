...
test('fifteen hand-drawn plants join the garden as kinds 9-23, each assembled from its own part files', () => {
  ...
  assert.ok([...kinds].some(k => k >= 9), 'the new kinds grow in the later gardens');
  for (let k = 9; k < 20; k++) for (const growth of [0, .5, 1.2, 2.4]) g.drawGrowingFigmaPlant(plot({ kind: k, growth, x: 0 }), 40, 60, 1, 58);
});
