...
test('the same grow/climb rhythm works at 30, 60 and 120 Hz, including Insane',()=>{let elapsed=[];for(const hz of [30,60,120]){const g=setup('bulwark','insane').game,s=race(g,hz);assert.equal(g.runWon,true,'rate '+hz);elapsed.push(s.elapsed);}assert.ok(Math.max(...elapsed)-Math.min(...elapsed)<1,JSON.stringify(elapsed));});
